...
export const Scene7Extras: React.FC = () => {
  ...
  const fadeOut = interpolate(frame, [15 * fps, 17 * fps], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill style={{ opacity: fadeOut, display: "flex", flexDirection: "column", padding: "40px 80px", gap: 20 }}>
      ...
      <div style={{ flex: 1, display: "flex", gap: 20, alignItems: "center" }}>
        <Img src={staticFile("screenshots/06-calendario.png")} style={{ width: "50%", borderRadius: 12, boxShadow: "0 20px 40px rgba(0,0,0,0.4)", opacity: img1Opacity, translate: `0px ${img1Y}px`, objectFit: "contain" as const, maxHeight: 850 }} />
        <Img src={staticFile("screenshots/05-proyectos.png")} style={{ width: "50%", borderRadius: 12, boxShadow: "0 20px 40px rgba(0,0,0,0.4)", opacity: img2Opacity, translate: `0px ${img2Y}px`, objectFit: "contain" as const, maxHeight: 850 }} />
      </div>
    </AbsoluteFill>
  );
};
