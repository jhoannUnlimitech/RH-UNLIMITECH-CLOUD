 ...
 *
 * API:
 *   pom.lead_form                        → navigate to context node
 *   pom.lead_form(".cls", "[attr]")      → complement (appended directly, no spaces allowed)
 *   pom.lead_form.ready                  → state: [data-test-state="ready"]
 *   pom.lead_form._.name_section         → descend (space in CSS) to child
 *   pom.lead_form.ready.$()              → terminal: returns the CSS selector string
 *   pom.lead_form.$(' input.my-cls')     → terminal + raw CSS suffix (free-form, spaces allowed)
 *
 ...
 *   e.g. lead_form → "lead-form", first_name_input → "first-name-input"
 */
 ...
 * Callable for complements: pom.lead_form(".cls") returns same type.
 */
      ...
      if (childNode.type === 'key' && childNode.indexed) {
        ...
        return new Proxy(indexedProxy, {
          get(_fn, fnProp: string | symbol) {
            ...
            if (fnProp === 'length') {
              return () => `${attr.replace(`="${kebab}"`, `^="${kebab}-"`)}`;
            }
            return undefined;
          },
        });
