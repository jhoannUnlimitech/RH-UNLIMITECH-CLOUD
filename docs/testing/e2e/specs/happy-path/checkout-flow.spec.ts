...
/**
 * Checkout Flow — E2E Test (v3 — Escenario A with params)
 *
 * Tests the checkout transition page (CheckoutPage → Stripe → /paid).
 * Uses Escenario A (plan+interval params) to skip plan selection.
 *
 * v3 flow: /sign-up?plan=general&interval=year → general-info → details →
 *          thank-you → verify → agreement → agreement-signed → checkout → paid
 *
 * Timeout: 300s (5 min).
 *
 * Run:
 *   CDP_ENDPOINT=http://localhost:9223 npx playwright test checkout-flow
 */
...
e2e.describe.serial('Checkout Flow — General Membership (v3)', () => {
