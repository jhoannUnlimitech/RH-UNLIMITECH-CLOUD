...
/**
 * Paid Page — Happy Path E2E Test (v3)
 *
 * Verifies that after Stripe checkout, the paid page correctly shows
 * the payment success state with confirmation details.
 *
 * Timeout: 300s (5 min).
 *
 * Run:
 *   CDP_ENDPOINT=http://localhost:9223 npx playwright test paid-page
 */
...
e2e.describe.serial('Paid Page — Payment Success (v3)', () => {
  ...
  e2e('verify paid page success',      verifyPaidPage(getPage));
});
