 ...
 * Uso: npx ts-node --transpile-only video/screenshots/capture.ts
 */
...
async function capture() {
  ...
  // 12. CSW - Ver detalle de una solicitud
  ...
  // 13. Dashboard dark mode
  console.log('📸 Dashboard Dark Mode...');
  await page.goto(`${BASE_URL}/`);
  await page.waitForTimeout(1500);
  // Toggle dark mode
  const themeButton = await page.$('[class*="ThemeToggle"], button[aria-label*="theme"], button[aria-label*="modo"]');
  if (themeButton) {
